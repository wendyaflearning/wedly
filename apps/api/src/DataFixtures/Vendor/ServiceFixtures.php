...
class ServiceFixtures extends Fixture
{
    ...
    {
        // ── Services principaux ───────────────────────────────────────────────
        $this->create($manager, 'Lieu de réception',        'lieu-de-reception',        1,  VendorType::Lieu);
        $this->create($manager, 'Traiteur',                 'traiteur',                 2,  VendorType::Traiteur);
        $this->create($manager, 'Photographe',              'photographe',              3,  VendorType::Freelance);
        $this->create($manager, 'Vidéaste',                 'videaste',                 4,  VendorType::Freelance);
        $this->create($manager, 'Décoration',               'decoration',               5,  VendorType::Freelance);
        $this->create($manager, 'DJ',                       'dj',                       6,  VendorType::Freelance);
        $this->create($manager, 'Fleuriste',                'fleuriste',                7,  VendorType::Freelance);
        $this->create($manager, 'Maquillage',               'maquillage',               8,  VendorType::Freelance);
        $this->create($manager, 'Coiffure',                 'coiffure',                 9,  VendorType::Freelance);
        $this->create($manager, 'Coordinatrice de mariage', 'coordinatrice-mariage',    10, VendorType::Freelance);
        $this->create($manager, 'Tailleur homme',           'tailleur-homme',           11, VendorType::Freelance);
