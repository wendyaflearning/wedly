...
export async function generateMetadata({ params }: SpecialtiesPageProps): Promise<Metadata> {
  ...
  if (!resolved) return { title: 'Spécialités | Wedly' }

  return {
    title: `${resolved.service.name} — Spécialités | Wedly`,
    description: `Choisissez le style de ${resolved.service.name.toLowerCase()} qui vous ressemble.`,
  }
}
...
export default async function SpecialtiesPage({ params }: SpecialtiesPageProps) {
  ...
  const { service, primary } = resolved
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
              {tagValues.map((tagValue, index) => (
                <SpecialtyCard
                  ...
                  tagValue={tagValue}
                  index={index}
                  href={`/wedream-vendors/${slug}/${tagValue.id}`}
                />
              ))}
            </div>
