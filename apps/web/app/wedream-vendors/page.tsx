  ...
  return (
    <div className="bg-creme" style={{ fontFamily: "var(--font-manrope-var)" }}>
      ...
      <ScrollReveal>
        ...
        >
          Dix savoir-faire, un seul objectif&nbsp;: que ce mariage vous ressemble.
        </p>
      </ScrollReveal>

      <div id="categories" className="flex flex-col gap-10 px-6 pt-11 pb-2 md:block md:mx-auto md:max-w-[1320px] md:px-[clamp(20px,4vw,40px)] md:pt-[clamp(56px,7vw,88px)] md:pb-24">
        <ScrollReveal className="flex flex-col items-center gap-10 md:flex-row md:justify-center md:gap-[clamp(48px,6vw,100px)] md:mb-[clamp(56px,7vw,92px)]">
          {largeBubbles.map((bubble) => (
            <CategoryBubble key={bubble.slug} href={`/wedream-vendors/${bubble.slug}`} {...bubble} />
          ))}
        </ScrollReveal>

        <ScrollReveal className="grid grid-cols-2 gap-x-[22px] gap-y-9 justify-items-center md:grid-cols-4 md:gap-x-[clamp(32px,4vw,64px)] md:gap-y-[clamp(40px,4.5vw,60px)]">
      ...
      <ScrollReveal className="relative w-full aspect-[4/5] mt-14 md:mt-0 md:aspect-[21/9]">
        <Image
