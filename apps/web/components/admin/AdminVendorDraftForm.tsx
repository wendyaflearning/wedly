  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="sticky top-16 z-30 rounded-lg border border-bordeaux/10 bg-white px-4 py-3 shadow-sm lg:top-0">
        ...
        {notice && <p className="mt-3 rounded-md bg-[#f4fbf6] px-3 py-2 text-sm font-semibold text-[#286342]">{notice}</p>}
        {error && <p className="mt-3 rounded-md bg-[#fff0f0] px-3 py-2 text-sm font-semibold text-[#8a2f2f]">{error}</p>}
      </div>

      {sendResult && (
        <section className="rounded-lg border border-[#d8eadf] bg-[#f4fbf6] p-4">
          <div className="flex items-start gap-3">
            <Check size={22} className="mt-0.5 text-[#286342]" aria-hidden="true" />
            <div className="min-w-0 flex-1">
              <h2 className="text-base font-bold text-[#286342]">Invitation prête</h2>
              <div className="mt-3 flex flex-col gap-2 rounded-md border border-[#c9dfd0] bg-white p-3 md:flex-row md:items-center">
                <code className="min-w-0 flex-1 break-all text-xs text-texte">{sendResult.invitationUrl}</code>
                <button
                  type="button"
                  onClick={copyInvitationUrl}
                  className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-md bg-[#286342] px-3 text-sm font-semibold text-white"
                >
