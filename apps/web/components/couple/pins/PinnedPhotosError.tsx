...
/**
 * État erreur de la zone « Épinglés » : la lecture de
 * `GET /api/v1/couples/me/pins` a échoué (réseau ou API). Distinct de l'état
 * vide, qui lui est un succès sans épinglé.
 */
export function PinnedPhotosError() {
  return (
    <section className="rounded-2xl border border-danger-border bg-danger-soft px-6 py-12 text-center md:px-10 md:py-16">
      <h2 className="font-cormorant text-2xl font-medium text-danger md:text-[30px]">
        Vos photos épinglées n’ont pas pu être chargées
      </h2>
      <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-texte/70">
        Un problème temporaire nous empêche d’afficher vos épinglés. Réessayez dans un instant.
      </p>
      <Link
        href="/mon-espace/epingles"
